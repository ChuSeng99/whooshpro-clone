...
<template>
  <header class="fixed top-0 left-0 w-full bg-white shadow-md z-50">
    <div class="py-2 flex justify-between items-center">
      <div>
        <router-link to="/">
          <img src="../assets/images/logo.png" alt="Company Logo" class="max-h-16 min-w-16" />
        </router-link>
      </div>

      <button class="mobile-nav-toggle md:hidden" :aria-expanded="isNavVisible" @click="toggleNav">
        <span class="material-icons text-amber-600 hover:bg-amber-100 hover:rounded-lg p-1">
          {{ isNavVisible ? 'close' : 'menu' }}
        </span>
      </button>

      <nav class="primary-navigation flex gap-4" :data-visible="isNavVisible">
        <ul role="list" class="flex flex-col gap-4 text-white md:flex-row md:gap-0 md:text-black">
          <li
